...
void Adafruit_INA219::setCalibration_16V_6A(void)
{
  // By default we use a pretty huge range for the input voltage,
  // which probably isn't the most appropriate choice for system
  // that don't use a lot of power.  But all of the calculations
  ...
  // MaximumLSB = MaxExpected_I/4096
  // MaximumLSB = 0,0014             (1.4mA per bit)
  ...
  //
  // Max_Current = Current_LSB * 32767
  // Max_Current = 6.55A before overflow
  //
  // If Max_Current > Max_Possible_I then
  //    Max_Current_Before_Overflow = MaxPossible_I
  // Else
  ...
  ina219_powerDivider_mW = 4;     // Power LSB = 1mW per bit (4/1)
...
void Adafruit_INA219::begin(void) {
  ...
  setCalibration_16V_6A();	//setCalibration_32V_2A();
}
